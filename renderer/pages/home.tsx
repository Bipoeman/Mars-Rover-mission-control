  ...
  return (
    <div className={`${k2d.className} flex flex-col justify-between py-[5vh] h-dvh`}>
      ...
      <span className='flex justify-around mx-[10vw]'>
        <button onClick={onAcept} className='px-[8vw] py-6 bg-[#137140] hover:bg-[#125331] hover:transition-colors hover:duration-200 hover:ease-in-out text-3xl rounded-lg'>Accept</button>
        ...
        <button onClick={onToggleUpload} className={`px-6 py-6 bg-[#641371] hover:bg-[#4c1354] transition-transform hover:transition-colors hover:duration-200 hover:ease-in-out text-3xl rounded-lg`}>{`Re-Upload ${uploadAllowedState ? "Allowed 🔓" : "Denied 🔒"}`}</button>
      </span>
    </div>
  )
}
